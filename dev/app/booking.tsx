...
const getBackendUrl = () => {
  ...
  else if (Platform.OS === 'android') return 'http://10.0.2.2:8082';
  else return 'http://192.168.8.119:8082'; // Match your backend
  //else return 'http://10.31.23.48:8082';
};
...
const App = () => {
  ...
  const [bookedTimes, setBookedTimes] = useState<Record<string, string[]>>({}); // Format: { "2025-03-28": ["11:00:00"] }
  ...
  useEffect(() => {
    ...
    const fetchBookedTimes = async () => {
      try {
        const response = await axios.get(`${backendUrl}/api/appointments/booked`);
  ...
  const normalizeTime = (timeStr: string): string => {
    // Convert 12-hour format (e.g., "1:00 PM") to 24-hour format (e.g., "13:00:00")
    ...
    // Pad with leading zeros and append seconds
  ...
  const confirmAppointment = async () => {
    ...
    const normalizedTime = normalizeTime(selectedTime); // Convert to "HH:mm:ss"

    const appointmentData = {
      parentId: patientId || null,
    ...
    try {
      ...
      setBookedTimes((prev) => ({
        ...prev,
        [selectedDate]: [...(prev[selectedDate] || []), normalizedTime],
      }));
      Alert.alert(
        'Success',
  ...
  const isTimeBooked = (date: string, time: string) => {
    ...
    return bookedTimes[date]?.includes(normalizedTime) || false;
  };
  ...
  const submitReview = async () => {
    ...
    try {
      console.log('Posting review:', JSON.stringify(reviewData, null, 2));
      ...
      {page === 'home' ? (
        ...
        <ScrollView style={styles.fullPageContainer}>
          {page === 'doctorList' ? (
            ...
            <>
              <Text style={styles.sectionTitle}>{selectedDoctor?.name || 'No Doctor Selected'}</Text>
            ...
            <>
              <Text style={styles.sectionTitle}>Reviews for {selectedDoctor?.name || 'No Doctor Selected'}</Text>
