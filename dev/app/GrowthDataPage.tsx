...
import React, { useState, useEffect } from 'react';
import { View, Text, ScrollView, StyleSheet, Alert, Dimensions, TextInput } from 'react-native';
import axios from 'axios';
import { LineChart } from 'react-native-chart-kit';
...
const GrowthDataPage = () => {
  const [growthData, setGrowthData] = useState<GrowthDataEntry[]>([]);
  const [filteredGrowthData, setFilteredGrowthData] = useState<GrowthDataEntry[]>([]);
  ...
  useEffect(() => {
    const fetchGrowthData = async () => {
      try {
        const response = await axios.get('http://localhost:8082/api/growth-data');
        setGrowthData(response.data);
        setFilteredGrowthData(response.data); // Initially, show all data
      } catch (error) {
        console.error('Error fetching growth data:', error);
        Alert.alert('Error', 'Failed to fetch growth data. Check the backend logs.');
      }
  ...
  const handleSearch = (text: string) => {
    setSearchDate(text);
    if (text === '') {
      setFilteredGrowthData(growthData); // Show all data if search is empty
    } else {
      const filteredData = growthData.filter((data) =>
        data.date.includes(text) // Filter by date (you can customize the comparison logic if needed)
      );
      setFilteredGrowthData(filteredData);
    }
  };

  const weightData = filteredGrowthData.map((entry) => parseFloat(entry.weight));
  const heightData = filteredGrowthData.map((entry) => parseFloat(entry.height));
  const bmiData = filteredGrowthData.map((entry) => parseFloat(entry.bmi));
  const ageLabels = filteredGrowthData.map((entry) => entry.age); // X-axis (Age)
